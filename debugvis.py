...
def plot_matrix(image, xdim, ydim, cmap='Greys'):
    ...
    data = np.frombuffer(data, dtype=np.float32).reshape((ydim,xdim))
    plt.imshow(data, cmap=cmap, interpolation='nearest')
    show()
    return True

def plot_image(image, xdim, ydim, cmap='Greys'):
    image = debugger.unwrap(image)
    if image.TypeIsPointerType():
        image_addr = image.GetValueAsUnsigned()
    else:
        image_addr = image.AddressOf().GetValueAsUnsigned()
    data = lldb.process.ReadMemory(image_addr, int(xdim * ydim) * 4, lldb.SBError())
    data = np.frombuffer(data, dtype=np.uint8).reshape((ydim,xdim, 4))
