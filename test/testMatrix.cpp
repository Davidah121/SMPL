...
{
	SECTION("Test Basic Matrix Operations")
	{
		...
		smpl::MatrixF B = smpl::MatrixF(8, 8); B.setAllValues(2);
		smpl::MatrixF C = A + B;
		REQUIRE(C.getRows() == 8);
		...
		for(int i=0; i<8; i++)
		{
			for(int j=0; j<8; j++)
			{
				REQUIRE(C[i][j] == (((float)i*j)/2));
			}
		}
	}

	SECTION("Test Broadcast Matrix Operations")
	{
		smpl::MatrixF A = smpl::MatrixF(8, 8); //64 values
		for(int i=0; i<8; i++)
		{
		...
		smpl::MatrixF C = A.broadcastAdd(17);
		REQUIRE(C.getRows() == 8);
		REQUIRE(C.getColumns() == 8);
		for(int i=0; i<8; i++)
		{
			for(int j=0; j<8; j++)
			{
				REQUIRE(C[i][j] == (i*j + 17));
			}
		}

		C = A.broadcastSubtract(31);
		REQUIRE(C.getRows() == 8);
		REQUIRE(C.getColumns() == 8);
		for(int i=0; i<8; i++)
		{
			for(int j=0; j<8; j++)
			{
				REQUIRE(C[i][j] == (i*j - 31));
			}
		}

		C = A.broadcastInverseSubtract(29);
		REQUIRE(C.getRows() == 8);
		REQUIRE(C.getColumns() == 8);
		for(int i=0; i<8; i++)
		{
