...
namespace smpl
{
	...
	public:
		...
		*/
		HttpServer(NetworkConfig configuration, int threads, bool useHTTPS = false, const std::string& certificateFile = "", const std::string& keyFile = "");
		...
		* @param range 
		*/
		void sendRangeError(size_t id, const std::string& range);
		...
		*/
		void setOptionsForURL(const std::string& url, unsigned int allowedRequestFlags, const std::string& allowedCustomHeaders);
		...
		*/
		template<typename T>
		int receiveMessage(T& msg, size_t id)
		{
			//send raw bytes
			if(conn != nullptr)
			{
		...
		*/
		template<typename T>
		int receiveMessage(T* msg, int elements, size_t id)
		{
			//send raw bytes
			if(conn != nullptr)
			{
		...
		*/
		 ...
		 * @return char 
		 */
		char staggeredSend(size_t id, const unsigned char* buffer, size_t startPoint, size_t endPoint);
		 ...
		 * @return char 
		 */
		char staggeredSendFile(size_t id, const File& f, size_t startPoint, size_t endPoint);
	...
	private:
		...
		// HybridSpinLock idUsageMutex;
		std::mutex idUsageMutex;
		SimpleHashSet< size_t > idInUse;
		SimpleHashMap<size_t, size_t> connTimeValues;
		SimpleHashMap<size_t, size_t> jobSubmitTValues;
		SimpleHashMap<size_t, size_t> jobStartTValues;
		...
		void onConnection(const std::string& ip, size_t id);
		void onDataArrived(const std::string& ip, size_t id);
		void onDisconnection(const std::string& ip, size_t id);
