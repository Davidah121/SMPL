...
namespace smpl
{
	...
	public:
		...
		//load / store
		static std::pair<SIMD_SSE<float>, SIMD_SSE<float>> loadFP16(const Float16* pointer)
		{
	...
	public:
		...
		SIMD_SSE<double> blend(const SIMD_SSE<double>& other, const SIMD_SSE<double>& blendFactor) const
		{
		...
		operator SIMD_SSE<int64_t>() const;
	...
	public:
		...
		//load / store
		static SIMD_AVX<float> loadFP16(const Float16* pointer)
		{
		...
		SIMD_AVX<float> blend(const SIMD_AVX<float>& other, const SIMD_AVX<float>& blendFactor) const
		{
		...
		float sum() const
		{
			SIMD_SSE<float> lows = _mm256_extractf128_ps(values, 0);
			SIMD_SSE<float> highs = _mm256_extractf128_ps(values, 1);
			return lows.sum() + highs.sum();
		}

		operator SIMD_AVX<int>() const;
	...
	public:
		...
		SIMD_AVX<double> blend(const SIMD_AVX<double>& other, const SIMD_AVX<double>& blendFactor) const
		{
		...
		{
			double arr[4];
			__m256d temp = _mm256_permute2f128_pd(values, values, 0b00000001); //contains A3, A4, A1, A2
			temp = _mm256_add_pd(values, temp); //contains A1+A3, A2+A4, ...
			temp = _mm256_hadd_pd(temp, temp); //contains A1+A2+A3+A4, ...
			return _mm256_cvtsd_f64(temp);
		}

		operator SIMD_AVX<int64_t>() const;
