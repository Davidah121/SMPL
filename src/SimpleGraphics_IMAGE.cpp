...
namespace smpl
{
	...
	void SimpleGraphics::drawImage(Image* img, int x, int y, Image* surf)
	{
		...
		for(int tY=minY; tY<maxY; tY++)
		{
			...
			while(tX < stopPoint)
			{
				SIMD_U32 src = SIMD_U32::load((unsigned int*)&drawImgPixels[imgX + imgY*img->getWidth()]);
				SIMD_U32 dest = SIMD_U32::load((unsigned int*)&surfPixels[tX + tY*tempWidth]);
				SIMD_U32 blendV = blend(src.values, dest.values);
				blendV.store((unsigned int*)&surfPixels[tX + tY*tempWidth]);
	...
	void SimpleGraphics::drawSprite(Image* img, int x, int y, Image* surf)
	{
		...
		for(int tY=minY; tY<maxY; tY++)
		{
			...
			while(tX < stopPoint)
			{
				SIMD_U32 src = SIMD_U32::load((unsigned int*)&drawImgPixels[u + v*img->getWidth()]);
				SIMD_U32 dest = SIMD_U32::load((unsigned int*)&surfPixels[tX + tY*tempWidth]);
	...
	//Gather operations help with this.
	...
	void SimpleGraphics::drawSpritePart(Image* img, int x, int y, int imgX, int imgY, int imgW, int imgH, Image* surf)
	{
		...
		for(int tY=minY; tY<=maxY; tY++)
		{
			...
			while(tX < stopPoint)
			{
				SIMD_U32 srcC = SIMD_U32::load((unsigned int*)&drawImgPixels[u + v*img->getWidth()]);
				SIMD_U32 destC = SIMD_U32::load((unsigned int*)&surfPixels[tX + tY*tempWidth]);
				SIMD_U32 srcMultiplied = multColor(srcC.values, activeColorAsSIMD.values);
				SIMD_U32 blendedColor = blend(srcMultiplied.values, destC.values);
				blendedColor.store((unsigned int*)&surfPixels[tX + tY*tempWidth]);
				tX += SIMD_U32::SIZE;
				u += SIMD_U32::SIZE;
			}
