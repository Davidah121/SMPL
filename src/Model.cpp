...
namespace smpl
{
	...
	std::vector<int> Model::getVertex(size_t i)
	{
		...
		if(indexed)
		{
			for(size_t j=0; j<formatInfo.size(); j++)
			{
				...
				{
					rValues.push_back( vertexData[j][index + k]);
				}
			}
		}
		else
	...
	Triangle2DModel Model::convertTo2DTriModel()
	{
		...
		case Model::TRIANGLES:
			...
			{
				std::vector<int> vertInfo = getVertex(i-2);
				std::vector<int> vertInfo2 = getVertex(i-1);
				std::vector<int> vertInfo3 = getVertex(i);

				float* positionData1 = (float*)&vertInfo[offset];
				float* positionData2 = (float*)&vertInfo2[offset];
				float* positionData3 = (float*)&vertInfo3[offset];
		...
		case Model::TRIANGLE_FAN:
			...
			{
				std::vector<int> vertInfo = getVertex(0);
				std::vector<int> vertInfo2 = getVertex(i-1);
				std::vector<int> vertInfo3 = getVertex(i);
		...
		case Model::QUADS:
			...
			for(int i=3; i<amtVert; i+=4)
			{
				...
				float* positionData2 = (float*)&vertInfo2[offset];
				float* positionData3 = (float*)&vertInfo3[offset];
				float* positionData4 = (float*)&vertInfo4[offset];
